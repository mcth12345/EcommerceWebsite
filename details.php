  ...
  <main>
    ...
    <div class="nero">
      <div class="nero__heading">
        <span class="nero__bold">CHI TIẾT </span>SẢN PHẨM
      </div>
      <p class="nero__text">
      </p>
    </div>
...
else if($product_size =="Vừa"){
  $product_size =2;
}

else if($product_size =="Lớn"){
  $product_size =3;
}


$check_product2 = "select count(*) as sl from product_size where MaSize=$product_size AND product_id=$p_id and qyt >= $product_size";
echo "<script>alert('$check_product2')</script>";
$run_check2 = mysqli_query($con,$check_product2);

$row_products = mysqli_fetch_array($run_check2);
...
if($sl<=0){
  echo "<script>alert('Số lượng kho không đủ')</script>";
...
<select name="product_qty" class="form-control form-control-lg" >
...
<select name="product_size" class="form-control form-control-lg" >
...
Giá sản phẩm: $pro_price.000 VNĐ
...
<h3 class="text-center"> Bạn có thể sẽ thích những sản phẩm này </h3>
